...
bool loadMedia()
{
	...
	gBackground = loadTexture( "background.png" );
		...
		gButton = loadTexture( "button.png" );
				if( gButton == NULL )
				{
					printf( "Failed to load texture image!\n" );
					success = false;
	...
	else
	{
		...
		{	
			...
			while (!quit) {
				...
				while (SDL_PollEvent(&e) != 0) {
					...
					if (e.type == SDL_QUIT) {
						...
						case SDL_MOUSEBUTTONDOWN:
							if (e.button.x > 440 && e.button.x < 610
									&& e.button.y > 400 && e.button.y < 450) {
								degrees += 10;
							}
							break;
				...
				arrow.w = 100;
				arrow.h = 100;
				gArrowTexture.render((SCREEN_WIDTH/2) -50,(SCREEN_HEIGHT/2)-50, &arrow, degrees, NULL, flipType);


				SDL_Rect button;
				button.x =SCREEN_WIDTH - 200 ;
