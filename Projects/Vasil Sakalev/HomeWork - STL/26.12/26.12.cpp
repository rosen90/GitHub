...
int main() {
	...
	Mmid sigleWord;
	int count = 0;
	for (list<string>::iterator i = uniqueWords.begin(); i != uniqueWords.end();
			i++) {
		string temp = *i;
		for (list<string>::iterator j = wordsList.begin(); j != wordsList.end();
				j++) {
			string temp2 = *j;
			if (temp == temp2) {
				count++;
	...
	for (Mmid::const_iterator iter = sigleWord.begin(); iter != sigleWord.end();
			++iter) {
		if (iter->second != " ") {
