...
private:
	void recursivePrintReverseFunction( ListNode< NODETYPE > * ) const;
...
template< typename NODETYPE >
void List< NODETYPE >::recursivePrint() const
{
	cout << "The list printed in reverse order is :\n";
	recursivePrintReverseFunction( firstPtr );
	cout << '\n';
} // end function recursivePrintReverse

// helper for printing a list backward recursively.
template< typename NODETYPE >
void List< NODETYPE >::recursivePrintReverseFunction(
ListNode< NODETYPE > *currentPtr ) const
{
	if ( currentPtr == 0 )
	return;
	recursivePrintReverseFunction( currentPtr->getNextPtr() );
